<?php
  ...
  $listado = @$conexionB->obtenerlista($querysB->getListadoEstatus());
  $totRegs = $conexionB->numregistros();

  if($totRegs > 0){
?>
<table id="listEstatus" class="table table-bordered table-striped">
  <thead>
    <tr>
  ...
  <?php foreach ($listado as $key) { ?>
    <tr>
      ...
      <td class="text-center">
        <button type="button" class="btn btn-success btn-sm" onclick="ModRegEstatusPropiedades(<?= $key->id_estatus ?>,'<?= $key->nombre ?>','<?= $key->icono ?>');"><i class="fa fa-edit"></i></button>
        <button type="button" class="btn btn-danger btn-sm" onclick="ModRegEstatusPropiedades(<?= $key->id_estatus ?>,'<?= $key->nombre ?>','<?= $key->icono ?>',1);"><i class="fa fa-trash"></i></button>
      </td>
    </tr>
  <?php } ?>
  </tbody>

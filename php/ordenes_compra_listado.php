...
<?php
  ...
  $aTipoComp = array(0 => 'Credito', 1 => 'De contado');
  $aEstatus  = array(0 => 'En captura', 1 => 'En espera', 2 => 'En proceso', 3 => 'Autorizada', 4 => 'Concluida', 5 => 'Cancelada');
  exit($querys->ordenes_compra_listado());
  @$conexion->obtenerlista($querys->ordenes_compra_listado());
  $totRegistros = $conexion->numregistros();

  $limite = 10;
  $inicio = ($pagina - 1) * $limite;
  $totalPaginas = ceil($totRegistros / $limite);
  $listado = @$conexion->obtenerlista($querys->ordenes_compra_listado()." LIMIT ".$inicio.','.$limite);
  ...
  ?>
    <tr>
      <td class="text-left v-align pt_2em">
        ...
        <p class="mg-1em"><label>OBRA:</label> <?= $key->nombre_obra ?></p>
